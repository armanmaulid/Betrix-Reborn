...
| 7 — Library Applicability Audit                 | 🟢 7/37 applied (3 top + 4 minor); 1 false positive; 1 explicit deferral (P-13)         | `e2024f1` + `6b23a21`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Top: B-1 (admin plugin), U-1 (isShuttingDownLease), D-1+B-7 (BA migration + 3 columns). Minor: D-2 (pg.Pool timeouts), U-9 (cron timezone), U-5 (Redis auto-serialize consistency), B-3 (BA cross-origin cookies). T-1 false positive. See `docs/audit-library-applicability-2026-08-30.md` §9 + §10. |
...
**Audit round 6 fixes applied (`84f508d`):** F-2+T-2 (drop `disableRequestLogging: true` from `LogController`, silences FSTDEP023 + de-duplicates with `isLogDisabled` override), F-3 (expanded `forceCloseConnections: 'idle'` comment to clarify active-SSE streams are NOT closed by this option — they go through `ssePlugin.preClose` → `sseHub.closeAll()`), T-5 (3-line rationale comment on 7 `as DTO` cast sites explaining why intentional), U-2 (clear prod credentials error in `createRedisClient` if Upstash env vars missing), U-4 (check `redis.expire` return value; best-effort retry on `0` = key missing). F-4 verified as no-fix-needed (env.TRUST_PROXY already boolean). T-3 verified as already fixed in round 4. See `docs/audit-library-applicability-2026-08-30.md` §14.
