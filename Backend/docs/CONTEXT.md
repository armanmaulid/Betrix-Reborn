...
| 6 — D1 `better-auth` | 🔵 Phase 0 ✅ Phase 1 ✅ Phase 2 Slice 1 ✅ | `d06677f` Phase 0 (env flag + dep + 4 BA tables + stub + DDL doc). `7af3616` Phase 1 (identity schema +updatedAt/image + d1-backfill-accounts.ts). `c6d9564` Phase 2 Slice 1 (full BA config via createAuth: emailAndPassword bcrypt cost 12, google placeholder, admin() plugin, rateLimit, trustedOrigins, additionalFields; betterAuth.plugin.ts mounts /api/auth/* only when USE_BETTER_AUTH=true; auth.plugin.ts authenticate flag-gated BA getSession vs legacy JWT). **Slice 2 (custom hooks: device/captcha/credit/audit) deferred.** See `docs/D1-better-auth-migration-plan.md`. |
