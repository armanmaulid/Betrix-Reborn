...
test.describe('E2E Broadcast & Maintenance Console', () => {
  ...
  test('should render broadcast center with target selectors and char counter', async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder*="URGENT: Scheduled Database Maintenance"]')).toBeVisible();
    await expect(page.locator('textarea[placeholder*="Enter complete bulletin message"]')).toBeVisible();
  });
