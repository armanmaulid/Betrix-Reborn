...
export function LiveGauges({ metrics, deltas, isLoading }: LiveGaugesProps) {
  ...
  const poolTotal = (data.dbPoolActive || 0) + (data.dbPoolIdle || 0);
  const poolActivePercent = poolTotal > 0 ? Math.round((data.dbPoolActive / poolTotal) * 100) : 0;
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between text-[11px] font-mono text-muted-foreground uppercase tracking-wider">
        <span className="flex items-center gap-1.5 text-accent font-bold">
          ...
          [LIVE GAUGES // 15S POLLING INTERVAL]
        </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 font-mono">
        ...
        <div className="border border-border bg-surface p-3 hover:border-accent/40 transition-colors">
          ...
          <div className="text-[9px] text-positive font-bold mt-1">FASTIFY 5 SERVER OK</div>
        </div>
      </div>
    </div>
  );
