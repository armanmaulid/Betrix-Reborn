...
import { useUserChatHistoryQuery } from '@/lib/queries/use-users';
...
export function UserChatHistory({ userId, userEmail }: UserChatHistoryProps) {
  ...
  // Handle both array response (when filtered by sessionId) and paginated response
  const isArrayResult = Array.isArray(data?.data);
  const messages: AdminChatMessage[] = Array.isArray(data)
    ? data
    : Array.isArray((data as any)?.data)
    ? (data as any).data
    : [];

  const meta = (data as PaginatedResult<AdminChatMessage>)?.meta || {
    page,
    limit,
                    ...
                    <div className="flex items-start sm:items-center gap-2.5">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-[10px] text-muted-foreground">
                          <span>
                            {item.createdAt
                              ? new Date(item.createdAt).toISOString().substring(0, 19).replace('T', ' ')
                              : 'N/A'}
                          </span>
                          <span>•</span>
                          <button
        ...
        <div className="border border-border bg-black p-3 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          ...
          <div className="flex items-center gap-3">
            <span className="text-muted-foreground">
              ...
              <strong className="text-foreground">{meta.totalPages}</strong> (TOTAL{' '}
              <strong className="text-foreground">{meta.total}</strong> INTERACTIONS)
            </span>
