...
import { useTestAgentMutation } from '@/lib/queries/use-agents';
import { useToast } from '@/components/ui/terminal-toast';
import { formatFinancialNumber } from '@/lib/utils';
import type { AiAgent, AgentTestResult } from '@/lib/types';
...
export function AgentTestConsole({ agent }: AgentTestConsoleProps) {
  ...
  const handleCopyReply = async () => {
    ...
    try {
      await navigator.clipboard.writeText(testResult.reply);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
      success('COPIED', 'Agent response copied to clipboard.');
    } catch {
      error('COPY FAILED', 'Unable to copy text to clipboard.');
    }
  };
