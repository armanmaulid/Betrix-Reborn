...
import { UpdateAdminUserSchema, type UpdateAdminUserInput } from '@/modules/operations/application/schemas/admin.schema';
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4 font-mono">
        ...
        {isEditingSelf && (
          <div className="border border-accent/40 bg-accent/10 p-3 text-[11px] text-accent flex items-start gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">
                You are managing your active root account. Self-demotion, suspension, and banning are locked to prevent loss of administrative access.
              </p>
            </div>
          </div>
        )}
          ...
          <div className={`border border-border bg-black p-3 flex items-center justify-between ${
            ...
            <label className={`relative inline-flex items-center ${isEditingSelf ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}>
              <input
                type="checkbox"
                {...register('isAdmin')}
                disabled={isEditingSelf}
